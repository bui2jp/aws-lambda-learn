...
```bash
sample-03-node22-image$ sam build
```
...
```bash
sample-03-node22-image$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sample-03-node22-image$ sam local start-api
sample-03-node22-image$ curl http://localhost:3000/
```
...
```bash
sample-03-node22-image$ sam logs -n HelloWorldFunction --stack-name sample-03-node22-image --tail
```
...
```bash
sample-03-node22-image$ cd hello-world
...
```bash
sam delete --stack-name sample-03-node22-image
```
